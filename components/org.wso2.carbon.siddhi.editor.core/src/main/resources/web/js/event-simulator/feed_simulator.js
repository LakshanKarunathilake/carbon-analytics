...
define([
  ...
  self.init = function(config) {
    ...
    self.$eventFeedForm = $("#event-feed-form");
    self.$eventFeedTab = $("#event-simulator ul.nav-tabs").find(
      'li a[aria-controls="event-feed-configs"]'
    );
    self.isDirty = false;
    ...
    self.propertyBasedGenerationOptionsForString = [
      "FIRST_NAME",
      "TIME_12H",
      "TIME_24H",
      "SECOND",
      ...
      invalidHandler: function(e, validator) {
        for (var i = 0; i < validator.errorList.length; i++) {
          $(validator.errorList[i].element)
            .closest('div.panel-collapse.collapse[id^="source_"]')
            .collapse("show");
        }
      }
    });
    ...
    $("#event-feed-form").on("submit", "form.feedSimulationConfig", function() {
      if (self.currentTotalSourceNum == 1) {
        ...
        if ($form.find('input[name="simulation-name"]').val() == "") {
          properties.simulationName = $form
            .find('input[name="simulation-name"]')
            .attr("placeholder");
        } else {
          properties.simulationName = $form
            .find('input[name="simulation-name"]')
        ...
        properties.startTimestamp = $form
          .find('input[name="start-timestamp"]')
          .val();
        properties.endTimestamp = $form
          .find('input[name="end-timestamp"]')
          .val();
        properties.noOfEvents = $form.find('input[name="no-of-events"]').val();
        properties.description = $form
          .find('textarea[name="feed-description"]')
          .val();
        properties.timeInterval = $form
          .find('input[name="time-interval"]')
          .val();
        ...
        $("div.sourceConfigs div.source").each(function() {
          ...
          var source = {};
          source.siddhiAppName = $sourceConfigForm
            .find('select[id="siddhi-app-name_' + uniqueId + '"]')
            .val();
          source.streamName = $sourceConfigForm
            .find('select[name="stream-name"]')
            .val();
          source.timestampInterval = $sourceConfigForm
          ...
          if ("csv" == sourceType) {
            ...
            source.fileName = $sourceConfigForm
              .find('select[name="file-name"]')
              .val();
            source.delimiter = $sourceConfigForm
              .find('input[name="delimiter"]')
              .val();
            if (
              $sourceConfigForm
              ...
              source.timestampAttribute = $sourceConfigForm
                .find('input[name="timestamp-attribute"]')
                .val();
              if (
                $sourceConfigForm.find('input[value="ordered"]').is(":checked")
              ) {
                source.isOrdered = true;
              }
              if (
                $sourceConfigForm
                  .find('input[value="not-ordered"]')
                  .is(":checked")
              ) {
            ...
            source.dataSourceLocation = $form
              .find('input[name="data-source-location"]')
              .val();
            ...
            $attributesDivs.each(function() {
              ...
              if (
                "custom" ==
                $attributesDiv.find('select[id^="attributes_"]').val()
              ) {
                attributeConfig.type = "CUSTOM_DATA_BASED";
                var valueList = $attributesDiv
                  .find('input[data-type="custom"]')
                  .val();
                ...
                var attDataType = $attributesDiv
                  .find('select[id^="attributes_"]')
                  .attr("data-type");
                if ("BOOL" == attDataType) {
                  attributeConfig.primitiveType = "BOOL";
                } else if ("STRING" == attDataType) {
                  attributeConfig.primitiveType = "STRING";
                  attributeConfig.length = $attributesDiv
                    .find('input[name$="_primitive_length"]')
                    .val();
                } else if ("INT" == attDataType || "LONG" == attDataType) {
                  attributeConfig.primitiveType = "INT";
                  attributeConfig.min = $attributesDiv
                    .find('input[name$="_primitive_min"]')
                    .val();
                  attributeConfig.max = $attributesDiv
                    .find('input[name$="_primitive_max"]')
                    .val();
                  ...
                  attributeConfig.max = $attributesDiv
                    .find('input[name$="_primitive_max"]')
                    .val();
                  attributeConfig.precision = $attributesDiv
                    .find('input[name$="_primitive_precision"]')
                    .val();
                }
              } else if (
                "property" ==
                ...
                attributeConfig.property = $attributesDiv
                  .find('select[name$="_property"]')
                  .val();
              } else if (
                "regex" ==
                $attributesDiv.find('select[id^="attributes_"]').val()
                ...
                attributeConfig.pattern = $attributesDiv
                  .find('input[name$="_regex"]')
                  .val();
              }
          ...
          Simulator.updateSimulation(
            ...
            function(data) {
              ...
              self.alertSuccess(
                "'" +
                  simulation.properties.simulationName +
                  "' feed updated. Please wait until feed" +
                  " list updated."
              );
          ...
          Simulator.uploadSimulation(
            ...
            function(data) {
              ...
              self.alertSuccess(
                "'" +
                  simulation.properties.simulationName +
                  "' feed saved. Please wait until feed" +
                  " list updated."
              );
              $("#create-simulation-modal-backdrop").remove();
              log.info(data);
    ...
    $("#event-feed-form").on(
      "click",
      ".feedSimulationConfig button.addNewSource",
      function() {
        $(".collapse").collapse();
        $("#addNewSourceError").addClass("hidden");
        var $sourceConfigs = $(this)
          .closest("form.feedSimulationConfig")
          .find("div.sourceConfigs");
        var sourceType = $(this)
          .closest("div.form-inline")
          .find("select.sources")
          .val();
        var sourcePanel = self.createConfigPanel(
          self.currentTotalSourceNum,
          self.dataCollapseNum,
          sourceType
        );
        $sourceConfigs.append(sourcePanel);
        var sourceForm = self.createSourceForm(
          sourceType,
          self.currentTotalSourceNum
        );
        var $sourceConfigBody = $sourceConfigs.find(
          "div.source[data-uuid=" +
            self.currentTotalSourceNum +
            "] div.panel-body"
        );
    ...
    $("#run_debug_app_modal").on("click", 'button[name="confirm"]', function() {
      ...
      var $panel = $("#active-simulation-list").find(
        'div.input-group[data-name="' + simulationName + '"]'
      );
      ...
      var activeTab = "";
      var $siddhiAppStartList = $(this)
        .closest("div.modal-content")
        .find("div.siddhi-app-list");
      var simulatingApps = {};
      ...
      for (var i = 0; i < simulationConfigs.length; i++) {
        ...
        if (siddhiAppName in simulatingApps) {
          ...
          if ("run" == simulatingApps[siddhiAppName]) {
            launcher = self.app.tabController
              .getActiveTab()
              .getSiddhiFileEditor()
              .getLauncher();
    ...
    self.$eventFeedConfigTabContent.on("click", "a i.fw-start", function() {
      ...
      var appName = "";
      var runDebugModalInitialContent =
        "<div class='clearfix'>" +
        "<div class='col-md-6'>" +
      ...
      Simulator.retrieveSiddhiAppNames(
        function(data) {
          var simulationConfigs =
            self.activeSimulationList[simulationName].sources;
          for (var j = 0; j < data.length; j++) {
            for (var i = 0; i < simulationConfigs.length; i++) {
              if (
                data[j]["siddhiAppName"] ==
                  simulationConfigs[i].siddhiAppName &&
                "STOP" == data[j]["mode"]
              ) {
                appName = data[j]["siddhiAppName"];
                stoppedAppAvailable = true;
                isValidApp = true;
                dynamicRunDebugContent += self.createRunDebugButtons(
                  data[j]["siddhiAppName"]
                );
                break;
              } else if (
          ...
          if (!isValidApp) {
            var message = {
              type: "ERROR",
              message:
                'Cannot Simulate Siddhi App "' +
                appName +
                '" as its in Faulty state.'
            };
            ...
            $siddhiAppList.append(
              runDebugModalInitialContent + dynamicRunDebugContent
            );
    ...
    self.$eventFeedConfigTabContent.on("click", "a i.fw-stop", function() {
      ...
      $panel
        ...
        .addClass("hidden");
      $panel
        .find("div.input-group-btn button.dropdown-toggle")
        .prop("disabled", false);
    });

    self.$eventFeedConfigTabContent.on(
      "click",
      'a[name="delete-source"]',
      function() {
        var $panel = $(this).closest(".input-group");
        var simulationName = $panel.attr("data-name");
        self.simulationDeleteModal = getSimulationDeleteConfirmation(
          simulationName
        );
        self.simulationDeleteModal.modal("show").on("shown.bs.modal");
        self.simulationDeleteModal.on("hidden.bs.modal");
        var deleteWizardError = self.simulationDeleteModal.find(
          "#deleteWizardError"
        );
        deleteWizardError.hide();
        self.simulationDeleteModal
          .find("button")
          .filter("#deleteButton")
          .click(function() {
            Simulator.deleteSimulation(
              ...
              function(data) {
                ...
                self.alertSuccess(
                  "Simulation '" +
                    simulationName +
                    "' deleted. Please wait until feed list" +
                    " updated"
                );
              },
              function(data) {
                deleteWizardError.text(
                  "Simulation '" +
                    simulationName +
    ...
    $("#event-feed-configs").on(
      "click",
      'button[name="create-new-config"]',
      function() {
        self.clearEventFeedForm();
        $("#addNewSourceError").addClass("hidden");
        if ("create" == self.$eventFeedForm.attr("mode")) {
          ...
          $("#event-feed-form")
            .find('select[name="sources"]')
            .val("Random");
          $(
            '<div class="modal-backdrop fade in" id="create-simulation-modal-backdrop"></div>'
          ).appendTo("body");
        }
      }
    );

    $("#clear_confirmation_modal_for_create").on(
      "click",
      'button[name="confirm"]',
      function() {
        self.clearEventFeedForm();
      ...
      function() {
        ...
        if ("edit" == $eventFeedForm.attr("mode")) {
          ...
          if (editingActiveSimulation) {
            self.activeSimulationList[
              self.getValue(simulationConfig.properties.simulationName)
            ].editMode = true;
          } else {
            self.inactiveSimulationList[
              self.getValue(simulationConfig.properties.simulationName)
        ...
        for (var i = 0; i < sources.length; i++) {
          ...
          self.bindDynamicContent(
            sourceForm,
            sourceSimulationType,
            self.currentTotalSourceNumi
          );
          self.loadSiddhiAppNamesAndSelectOption(
            self.currentTotalSourceNum,
            source
          );
        }
    ...
    $("#event-feed-form").on(
      "change",
      '.sourceConfigs div select[name="siddhi-app-name"]',
      function() {
        var $element = $(this);
        ...
        $siddhiAppMode.html(
          "mode : " + self.siddhiAppDetailsMap[siddhiAppName]
        );
      ...
      function() {
        ...
        Simulator.retrieveStreamAttributes(
          $sourceConfigForm
            ...
            .val(),
          $sourceConfigForm.find('select[name="stream-name"]').val(),
          function(data) {
    ...
    $("#event-feed-form").on(
      "change",
      '.sourceConfigs div select[name="table-name"]',
      function() {
        var $element = $(this);
        self.loadColumnNames(
          $element.closest(".sourceConfigForm"),
          $element.val()
        );
      }
    );
    ...
    $("form#csv_upload_modal_form").on("click", "button.upload-csv", function(
      e
    ) {
    ...
    $("#event-feed-form").on(
      "click",
      'button[name="loadDbConnection"]',
      function() {
        var $sourceConfigForm = $(this).closest("div.sourceConfigForm");
        self.selectedSourceNum = $sourceConfigForm.attr("data-uuid");
        $sourceConfigForm
          .find(".connectionSuccessMsg")
          .html(self.generateConnectionMessage("connecting"));
        var connectionDetails = self.validateAndGetDbConfiguration(
          $sourceConfigForm
        );
        if (null != connectionDetails) {
          ...
          Simulator.testDatabaseConnectivity(
            ...
            function(data) {
              self.refreshTableNamesFromDataSource(
                connectionDetails,
                $tableNames
              );
              $sourceConfigForm
            ...
            function(msg) {
              ...
              $sourceConfigForm
                .find(".connectionSuccessMsg")
                .html(self.generateConnectionMessage("failure"));
            }
          );
        }
      }
    ...
    $("#event-feed-form").on(
      ...
      function() {
        ...
        $sourceConfigForm
          .find('[class^="feed-attribute-random-' + dynamicId + '-property"]')
          .each(function() {
            $(this).prop("selectedIndex", -1);
          });
        // Get all input elements of new attribute and add validation rule
        var inputs = this.closest("div.form-group").getElementsByTagName(
          "input"
        );
  ...
  self.refreshSiddhiAppList = function($siddhiAppSelect, siddhiAppNames) {
    var initialOptionValue =
      '<option value = "-1" disabled>-- Please Select a Siddhi App --</option>';
    var newSiddhiApps = self.generateOptions(
      siddhiAppNames,
      initialOptionValue
    );
  ...
  self.selectSourceOptions = function(
    $siddhiAppSelect,
    siddhiAppName,
    initialLoading
  ) {
    ...
    if (siddhiAppName in self.siddhiAppDetailsMap) {
      ...
      if (siddhiAppName !== null) {
        ...
        $form.find('div[data-name="siddhi-app-name-mode"]').empty();
        $form
          .find('select[name="stream-name"]')
          .empty()
          .prop("disabled", true);
        ...
        $form
          ...
          .prop("checked", false);
        $form.find('input[name="timestamp-attribute"]').empty();
        $form
          .find('input[name="timestamp-option"][value="interval"]')
          .prop("checked", true);
        $form.find('input[name="timestamp-interval"]').val("1000");
        $form.find('input[name="ordered"][value="true"]').prop("checked", true);
        $form
  ...
  self.bindDynamicContent = function(sourceForm, type, uniqueId) {
    if (type == "Random") {
      sourceForm
        .find("a[id='randomAdvanceConfigToggle_" + uniqueId + "']")
        .on("click", function() {
          if ($(this).hasClass("active")) {
            $(this).removeClass("active");
          } else {
      ...
      sourceForm
        .find("button[id='upload-csv-file_" + uniqueId + "']")
        .on("click", function() {
          var $element = $(this);
          var $div = $element.closest(".sourceConfigForm");
      ...
      sourceForm
        .find("a[id='csvAdvanceConfigToggle_" + uniqueId + "']")
        .on("click", function() {
          if ($(this).hasClass("active")) {
            $(this).removeClass("active");
          } else {
  ...
  self.createConfigPanel = function(
    totalSourceNum,
    dataCollapseNum,
    sourceType
  ) {
  ...
  self.createSourceForm = function(sourceType, totalSourceNum) {
    switch (sourceType) {
      case "CSV file":
        ...
        csvTemplate.html(
          csvTemplate.html().replaceAll("{{dynamicId}}", totalSourceNum)
        );
      ...
      case "Database":
        ...
        dbTemplate.html(
          dbTemplate.html().replaceAll("{{dynamicId}}", totalSourceNum)
        );
      ...
      case "Random":
        ...
        randomTemplate
          .find("a")
          .attr("id", "randomAdvanceConfigToggle_" + totalSourceNum);
        randomTemplate
          .find('div[id="randomAdvanceContent_dynamicId"]')
          .attr("id", "randomAdvanceContent_" + totalSourceNum);
        randomTemplate.html(
          randomTemplate.html().replaceAll("{{dynamicId}}", totalSourceNum)
        );
    ...
    $(".sourceConfigs div.source").each(function(i) {
      ...
      if (uuid > removedUUID) {
        ...
        $source
          ...
          .contents()
          .last()
          .replaceWith("Source " + uuid + " - " + match[2]);
      }
    });
  };
  ...
  self.loadSiddhiAppNames = function(elementId) {
    ...
    Simulator.retrieveSiddhiAppNames(
      function(data) {
        ...
        self.refreshSiddhiAppList(
          $siddhiAppSelect,
          Object.keys(self.siddhiAppDetailsMap)
        );
  ...
  self.loadSiddhiAppNamesAndSelectOption = function(elementId, source) {
    console.log("this is method is console");
    var $siddhiAppSelect = $(
      'div[data-uuid="' +
        elementId +
        '"] select[id="siddhi-app-name_' +
    ...
    var $siddhiAppMode = $(
      'div[data-uuid="' + elementId + '"] div[data-name="siddhi-app-name-mode"]'
    );
    var $streamNameSelect = $(
      'div[data-uuid="' + elementId + '"] select[name="stream-name"]'
    );
    var siddhiAppName = $siddhiAppSelect.val();
    Simulator.retrieveSiddhiAppNames(
      function(data) {
        ...
        if (source.siddhiAppName in self.siddhiAppDetailsMap) {
          ...
          $siddhiAppMode.html(
            "mode : " + self.siddhiAppDetailsMap[source.siddhiAppName]
          );
                ...
                Simulator.retrieveStreamAttributes(
                  ...
                  function(data) {
                    ...
                    if ("CSV_SIMULATION" == source.simulationType) {
                      ...
                      self.loadCSVFileNamesAndSelectOption(
                        self.totalSourceNum,
                        source.fileName
                      );
                      var $timestampIndex = $sourceConfigForm.find(
                        'input[value="attribute"]'
                      );
                      var $timestampInteval = $sourceConfigForm.find(
                        'input[value="interval"]'
                      );
                      var $ordered = $sourceConfigForm.find(
                        'input[value="ordered"]'
                      );
                      var $notordered = $sourceConfigForm.find(
                        'input[value="not-ordered"]'
                      );
                      var $timestampAttribute = $sourceConfigForm.find(
                        'input[name="timestamp-attribute"]'
                      );
                      var $timeInterval = $sourceConfigForm.find(
                        'input[name="timestamp-interval"]'
                      );
                      if (
                        source.timestampInterval &&
                        0 != source.timestampInterval.length
                      ) {
                      ...
                      $sourceConfigForm
                        .find('input[name="delimiter"]')
                        .val(source.delimiter);
                      self.addSourceConfigValidation(
                        source.simulationType,
                        self.currentTotalSourceNum
                      );
                      ...
                      var $timestampInterval = $sourceConfigForm.find(
                        'input[value="interval"]'
                      );
                      var $timestampAttribute = $sourceConfigForm.find(
                        'input[name="timestamp-attribute"]'
                      ...
                      if (null != connectionDetails) {
                        var $tableNames = $sourceConfigForm.find(
                        ...
                        Simulator.testDatabaseConnectivity(
                          ...
                          function(data) {
                            ...
                            Simulator.retrieveColumnNames(
                              ...
                              source.tableName,
                              function(data) {
                                self.loadColumnNamesListAndSelect(
                                  data,
                                  $sourceConfigForm,
                                ...
                                if (
                                  ...
                                  var $timestampAtt = $sourceConfigForm.find(
                                    'select[name="timestamp-attribute"]'
                                  );
                                  $timestampAtt
                                    .find("option")
                                    .eq(
                          ...
                          function(msg) {
                            ...
                            $sourceConfigForm
                              .find(".connectionSuccessMsg")
                              .html(
                                self.generateConnectionMessage("editFailure")
                              );
                            var tableOption =
                              '<option value = "' +
                              source.tableName +
                      ...
                      "RANDOM_DATA_SIMULATION" == source.simulationType
                    ) {
                      var attributeConfiguration =
                        source.attributeConfiguration;
                      var $attributesDivs = $sourceConfigForm
                        .find(
                          'div.attributes-section label[for^="attributes_"]'
                        )
                        .closest("div");
                      ...
                      $attributesDivs.each(function() {
                        ...
                        var $attributesDiv = $(this);
                        var $attributeSelect = $attributesDiv.find(
                          'select[name^="attributes"]'
                        );
                        var attributeType = $attributeSelect.attr("data-type");
                        var attributeName = $attributeSelect
                          .attr("name")
                          .replaceAll("attributes_", "");
                        var id = this.id;
                        var $selectType = $attributesDiv.find(
                          'select[id^="attributes_"]'
                        );
                        if ("CUSTOM_DATA_BASED" == attributeConfig.type) {
                          $selectType
                            .find("option")
                            .eq(
                              $selectType.find('option[value="custom"]').index()
                            )
                            .prop("selected", true);
                          $sourceConfigForm
                          ...
                          $selectType
                            .find("option")
                            .eq(
                              $selectType.find('option[value="regex"]').index()
                            )
                            .prop("selected", true);
                          $sourceConfigForm
                            .find(".attributes_" + attributeName + "_config")
  ...
  self.loadCSVFileNames = function(dynamicId, initialLoading) {
    var $csvFileSelect = $(
      'div[data-uuid="' + dynamicId + '"] select[name="file-name"]'
    );
    ...
    Simulator.retrieveCSVFileNames(
      function(data) {
        ...
        $csvFileSelect
          ...
          .eq(
            $csvFileSelect
              .find('option[value="' + selectedFileName + '"]')
              .index()
          )
          .prop("selected", true);
      },
  ...
  self.refreshCSVFileList = function($csvFileSelect, csvFileNames) {
    ...
    if (csvFileNames.length == 0) {
      isNotUploaded = true;
      options +=
        '<option value = "-1" disabled>No Uploaded CSV file available</option>';
    }
  ...
  self.refreshStreamList = function($streamNameSelect, streamNames) {
    var initialOptionValue =
      '<option value = "-1" disabled>-- Please Select a Stream --</option>';
    var newStreamOptions = self.generateOptions(
      streamNames,
      initialOptionValue
    );
  ...
  self.refreshTableNamesFromDataSource = function(
    connectionDetails,
    $tableNames
  ) {
    Simulator.retrieveTableNames(
  ...
  self.addSourceConfigValidation = function(sourceType, dynamicId) {
    ...
    $sourceConfigForm
      .find('select[id="siddhi-app-name_' + dynamicId + '"]')
      .rules("add", {
        required: true,
        messages: {
  ...
  self.addAllSourceValuesValidation = function() {
    $("#source-accordion div.attributes-section input[type=text]").each(
      function() {
        $(this).rules("add", {
          required: true,
          messages: {
  ...
  self.refreshAttributesList = function(uuid, streamAttributes) {
    var $attributesDiv = $(
      'div.sourceConfigForm[data-uuid="' + uuid + '"] div.attributes-section'
    );
    var dataType = $("div.sourceConfigForm[data-uuid=" + uuid + "]").attr(
      "data-type"
    );
    $attributesDiv.html(self.generateAttributesDivForSource(dataType));
    var attributes = self.generateAttributesListForSource(
      dataType,
      streamAttributes,
      uuid
    );
    ...
    if (dataType == "random") {
      for (var i = 0; i < streamAttributes.length; i++) {
        ...
        $attributesDiv
          .find('select[id="' + dynamicSelectBoxId + '"]')
          .val("primitive")
          .change();
      }
    } else if (dataType == "db") {
      ...
      $timestampIndex.prop("checked", false);
      $timestampInteval.prop("checked", true);
      var $timeInterval = $sourceConfig.find(
        'input[name="timestamp-interval"]'
      );
      $timeInterval.val(1000);
      self.loadColumnNames(
        $sourceConfig,
        $sourceConfig.find('select[name="table-name"]').val()
      );
    } else {
      var $timestampIndex = $sourceConfig.find('input[value="attribute"]');
      var $timestampInteval = $sourceConfig.find('input[value="interval"]');
  ...
  self.generateAttributesListForSource = function(
    dataType,
    attributes,
    form_id = 1
  ) {
    ...
    for (var i = 0; i < attributes.length; i++) {
      ...
      switch (dataType) {
        ...
        case "db":
          ...
          break;
        case "random":
          temp = randomAttribute.replaceAll(
            "{{attributeName}}",
            attributes[i]["name"]
          );
          result += temp.replaceAll("{{attributeType}}", attributes[i]["type"]);
    ...
    result = result.replaceAll("{{id}}", form_id);
    console.log("result :", result);
    return result;
  };

  self.validateAndGetDbConfiguration = function($sourceConfigForm) {
    var dataSourceLocation = $sourceConfigForm
      .find('input[name="data-source-location"]')
      .val();
  ...
  //generate input fields for attributes
  self.loadColumnNamesListAndSelect = function(
    columnNames,
    $sourceConfigForm,
    selectedValueList
  ) {
    ...
    $sourceConfigForm.find(".feed-attribute-db").each(function() {
      ...
      $(this)
        ...
        .eq(
          $(this)
            .find('option[value="' + selectedValueList[i] + '"]')
            .index()
        )
        .prop("selected", true);
      i++;
  ...
  self.getCSVSimulationCongig = function($sourceCOnfig) {
    var source = {};
    source.simulationType = "CSV_SIMULATION";
    source.executionPlanName = $sourceCOnfig
  ...
  self.generateRandomAttributeConfiguration = function(
    randomType,
    attributeType,
    dynamicId,
    parentId
    ...
    switch (randomType) {
      case "custom":
        return self.generateCustomBasedAttributeConfiguration(
          attributeType,
          parentId
        );
      case "primitive":
        return self.generatePrimitiveBasedAttributeConfiguration(
          attributeType,
          parentId
        );
      case "property":
        return self.generatePropertyBasedAttributeConfiguration(
          attributeType,
          parentId
        );
      case "regex":
        return self.generateRegexBasedAttributeConfiguration(
          attributeType,
          parentId
        );
    }
  ...
  self.generateCustomBasedAttributeConfiguration = function(
    attrType,
    parentId
  ) {
    var staticValue = "";
  ...
  self.generatePrimitiveBasedAttributeConfiguration = function(
    attrType,
    parentId
  ) {
    var bool =
  ...
  self.generatePropertyBasedAttributeConfiguration = function(
    attrType,
    parentId
  ) {
    ...
    var propertyEndingTag = "</select>" + "</div>";
    ...
    for (var simulationName in self.activeSimulationList) {
      if (self.activeSimulationList.hasOwnProperty(simulationName)) {
        ...
        for (i = 0; i < activeSimulations.length; i++) {
          if (
            activeSimulations[i].properties.simulationName == simulationName
          ) {
            foundSimulaion = true;
          }
        }
        if (!foundSimulaion) {
          self.$eventFeedConfigTabContent
            .find('div[data-name="' + simulationName + '"]')
            .remove();
    ...
    for (var simulationName in self.inactiveSimulationList) {
      if (self.inactiveSimulationList.hasOwnProperty(simulationName)) {
        ...
        for (i = 0; i < inActiveSimulations.length; i++) {
          if (
            inActiveSimulations[i].properties.simulationName == simulationName
          ) {
            foundSimulaion = true;
          }
        }
  ...
  self.addActiveSimulationToUi = function(simulation) {
    ...
    if (simulationName in self.inactiveSimulationList) {
      self.$eventFeedConfigTabContent
        .find('div[data-name="' + simulation.properties.simulationName + '"]')
        .remove();
      delete self.inactiveSimulationList[simulationName];
    }
    if (
      0 ===
      self.$eventFeedConfigTabContent.find(
        'div[data-name="' + simulation.properties.simulationName + '"]'
      ).length
    ) {
      self.$eventFeedConfigTabContent
        .find('div[data-name="' + simulation.properties.simulationName + '"]')
        .remove();
      var simulationDiv =
        '<div class="input-group" data-name="' +
        simulation.properties.simulationName +
        '">' +
        ...
        "</div>";
      self.$eventFeedConfigTabContent
        .find("#active-simulation-list")
        .append(simulationDiv);
  ...
  self.addInActiveSimulationToUi = function(simulation) {
    ...
    if (
      ...
      var simulationDiv =
        ...
        simulation.properties.simulationName +
        '">' +
        '<span class="form-control" data-toggle="tooltip" title="' +
        simulation.exceptionReason +
        '">' +
        '<span class="simulation-name">' +
        simulation.properties.simulationName +
        "</span>" +
      ...
      self.$eventFeedConfigTabContent
        .find("#inactive-simulation-list")
        .append(simulationDiv);
    }
  };

  self.checkSimulationStatus = function(
    $panel,
    simulationName,
    isInitialStart
  ) {
    Simulator.getFeedSimulationStatus(
      ...
      function(data) {
        ...
        if (
          (!isInitialStart &&
            "STOP" === status &&
            "RUN" === self.activeSimulationList[simulationName].status) ||
          ...
          !(
            "STOP" === status &&
            "STOP" === self.activeSimulationList[simulationName].status
          )
        ) {
          setTimeout(function() {
  ...
  self.addDateTimePickers = function() {
    ...
    $startTimestamp.datetimepicker({
      ...
      showOn: "button",
      buttonText:
        '<span class="fw-stack"><i class="fw fw-square-outline fw-stack-2x"></i>' +
  ...
  self.convertDateToUnix = function() {
    ...
    if (
      self.siddhiAppDetailsMap[
        $form.find('select[name="siddhi-app-name"]').val()
      ] !== self.FAULTY
    ) {
  ...
  self.pollingSimulation = function() {
    if (
      self.$eventFeedTab.attr("aria-expanded") &&
      self.$eventFeedTab.closest("li").hasClass("active")
    ) {
      self.addAvailableFeedSimulations();
  ...
  self.addDynamicDefaultValues = function() {
    Simulator.getFeedSimulations(
      function(data) {
        ...
        self.form
          .find('input[name="simulation-name"]')
          .val("Feed Simulation " + ++self.numOfFeedSimulations);
  ...
  self.createRunDebugButtons = function(siddhiAppName) {
    var runDebugButtons =
      ...
      '<label class="siddhi_app_name col-md-6">' +
      siddhiAppName +
      "</label>" +
      ...
      if (self.activeSimulationList.hasOwnProperty(simulationName)) {
        ...
        for (var i = 0; i < simulationConfigs.length; i++) {
          ...
          if (
            ...
            (simulatorStatus == "RUN" || simulatorStatus == "PAUSE")
          ) {
            Simulator.simulationAction(
              simulationName,
              "stop",
              function(data) {
                self.alertWarning(
                  "'" +
                    simulationName +
                    "' simulation stopped due to termination of" +
                    " Siddhi app: " +
                    siddhiApp
                );
              },
              function(data) {
                self.alertError(
                  "'" +
                    simulationName +
                    "' simulation could not stopped when stopping" +
                    " Siddhi app: '" +
                    siddhiApp +
                    "'. Reason: " +
                    data
  ...
  function getSimulationDeleteConfirmation(simulationName) {
    return $(
      "<div class='modal fade' id='deleteSimulationModal' tabindex='-1' role='dialog' aria-tydden='true'>" +
        ...
        "Are you sure to delete Simulation Configuration: " +
        simulationName +
        "" +
        "</label>" +
        "</div>" +
